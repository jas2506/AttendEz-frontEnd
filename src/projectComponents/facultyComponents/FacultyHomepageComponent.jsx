...
function FacultyHomepageComponent({ c }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const res = await fetch(
            `${backendUrl}/faculty/liveAttendanceViewWithVersion?classCode=${classdetails.classCode}&lastVersion=${currentVersion}`,
            {
              method: "GET",
              headers: { Authorization: localStorage.getItem("facultyToken") },
            }
        );
        const data = await res.json();

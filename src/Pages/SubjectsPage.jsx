...
function getTodayAttendanceStats(attendance, timetable) {
  ...
  for (const day of weekdays) {
    ...
    for (const cls of dayClasses) {
      ...
      const endTime = { hour: endDate.getHours(), minute: endDate.getMinutes() }
...
function SubjectsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [attendanceData, timetableData] = await Promise.all([fetchAttendance(), fetchTimetable()])
  ...
  const handleViewCalendar = (classCode) => {
    const subjectName = timetable?.timetable?.classDetails?.[classCode]?.className || classCode
  ...
  const stats = getTodayAttendanceStats(attendance.attendance, timetable.timetable)
  ...
  const calculateAveragePercentage = () => {
    const totalAttended = stats.reduce((sum, subject) => sum + subject.attendedLectures, 0)
    const totalLectures = stats.reduce((sum, subject) => sum + subject.totalLectures, 0)
    return totalLectures > 0 ? ((totalAttended / totalLectures) * 100).toFixed(1) : 0
  }
      ...
      <div className="max-w-4xl mx-auto p-6 bg-white">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              {stats.map((subject, index) => {
                const percentage = calculatePercentage(subject.attendedLectures, subject.totalLectures)
                    ...
                    <tr key={index} className="hover:bg-blue-50 transition-colors duration-150">
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{subject.classCode}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="text-sm font-medium text-gray-900">{subject.attendedLectures}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="text-sm font-medium text-gray-900">{subject.totalLectures}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                      <span
          ...
          <div className="bg-blue-50 px-6 py-4 border-t border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">Based on all subjects combined</p>
              </div>
        ...
        <Dialog open={showCalendar} onOpenChange={setShowCalendar}>
          <DialogContent className="w-fit max-w-[90vw]">
            <DialogHeader>
              <DialogTitle>Attendance Calendar - {selectedSubject?.name}</DialogTitle>
              <DialogDescription>
            ...
            />
            <DialogClose asChild>
              <button className="mt-4 bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600">Close</button>
            </DialogClose>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showLectureDetails} onOpenChange={setShowLectureDetails}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Lecture Details - {selectedDate?.toLocaleDateString()}</DialogTitle>
              <DialogDescription>Individual lecture attendance for this day</DialogDescription>
            </DialogHeader>
              ...
              {selectedDayLectures.map((lecture, index) => (
                  <div
                      ...
                      className={`p-3 rounded-lg border ${
                          lecture.present === 1 ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"
                      }`}
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="font-medium text-sm">{lecture.time}</p>
                        <p className="text-xs text-gray-600">{lecture.lectureKey}</p>
                      </div>
                      <span
                          className={`px-2 py-1 rounded-full text-xs font-semibold ${
                              lecture.present === 1 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                          }`}
                      >
            ...
            <DialogClose asChild>
              <button className="mt-4 bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 w-full">Close</button>
            </DialogClose>
          </DialogContent>
        </Dialog>
      </div>

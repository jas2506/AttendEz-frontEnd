...
export default function CreateDeleteStudentPage() {
  ...
  const handleExcelUpload = (event) => {
    ...
    if (!selectedDegree || !selectedPassoutYear) {
      toast.error(
        "Please select degree, course, and passout year before uploading."
      );
      event.target.value = ""; // Reset file input so they can re-upload later
      return;
    }

    // ✅ Proceed if selections are valid
    ...
    reader.onload = (e) => {
      ...
      for (const row of jsonData) {
        ...
        if (!email || !registerNumber || !name) {
          toast.error("Skipping invalid row with missing fields.");
          continue;
        }

        const digitalid = email.split("@")[0].slice(-7);

        const isDuplicate = studentsToAdd.some(
          (s) => s.registerNumber === registerNumber
        );
        if (isDuplicate) {
          toast.warning(`Register number ${registerNumber} is already added.`);
          continue;
        }
      ...
      setStudentsToAdd((prev) => [...prev, ...parsedStudents]);
      toast.success(`${parsedStudents.length} students added from Excel.`);
    };
